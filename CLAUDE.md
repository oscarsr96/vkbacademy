...
| 9    | Deployment parcial (Vercel + Render + Neon)                     | ✅ Completado |
...
| `apps/api` (NestJS)     | Render (Docker)                      | Plan Starter |
| PostgreSQL              | Neon (serverless Postgres)           | Plan Free    |
...
| Render     | `FRONTEND_URL`                      | `https://<app>.vercel.app` (comas para múltiples) |
| Render     | `DATABASE_URL`                      | connection string de Neon (pooler si existe)      |
| Render     | `NODE_ENV`                          | `production`                                      |
| Render     | `JWT_SECRET` / `JWT_REFRESH_SECRET` | `openssl rand -hex 32`                            |

### Notas Render

- **Dockerfile Path:** `apps/api/Dockerfile`
- **Build Context:** `.` (raíz del repo — necesario para copiar `packages/shared`)
- Las migraciones **NO** corren en el contenedor. Se aplican desde un job del pipeline (`migrate-pre` / `migrate-prod` en `deploy-pipeline.yml`) vía `pnpm --filter @vkbacademy/api run migrate:deploy` con el `DATABASE_URL` del environment de GitHub, **antes** de desplegar la API. Así evitamos que una migración fallida tumbe el deploy y permite separar los permisos DB/deploy.
- El CMD del contenedor solo hace `node dist/main`.
- Cold start de Render Starter ~30-60s tras inactividad → el smoke test reintenta hasta 20× con 15s de espera entre cada uno.
...
| API (Render) | servicio API PRE (`srv-...`)  | servicio API PROD (`srv-...`) |
| PostgreSQL   | BD dedicada en Neon           | BD dedicada en Neon           |
...
| -------------- | -------------- | -------------------------------------- |
| `test`         | —              | —                                      |
| `migrate-pre`  | `pre`          | automático                             |
| `deploy-pre`   | `pre`          | automático                             |
| `smoke-pre`    | `pre`          | automático                             |
| `migrate-prod` | `prod-canary`  | required reviewer                      |
| `deploy-prod`  | `prod-canary`  | required reviewer (hereda environment) |
| `smoke-prod`   | `prod-canary`  | automático                             |
...
El deploy de Vercel PROD **no** se dispara desde el pipeline: la Git integration de Vercel detecta el push a main y despliega automáticamente. El pipeline solo verifica (`curl /`) que responde. Para PRE sí usamos `vercel deploy --prod` vía CLI.

### Secrets / variables requeridas

**Repository secrets:**

- `RENDER_API_KEY`, `RENDER_SERVICE_ID_PRE`, `RENDER_SERVICE_ID_PROD`
- `VERCEL_TOKEN`, `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID_PRE`, `VERCEL_PROJECT_ID_PROD`

**Environment secrets** (por cada entorno `pre` / `prod-canary`):

- `DATABASE_URL` — connection string de Neon apuntando a la BD de ese entorno
...
- **Render** despliega la nueva imagen y hace switch instantáneo (no es rolling entre replicas en plan Starter). El deploy anterior queda disponible para rollback manual desde el dashboard.
...
- El pipeline actual **no hace canary con traffic splitting real**. El environment `prod-canary` es solo un checkpoint con required-reviewer donde tú apruebas el deploy PROD tras ver PRE verde; el `promote-prod` posterior es otro reviewer gate que valida los smoke tests PROD antes de marcar la release como completada. Para canary con tráfico por porcentajes habría que migrar a una plataforma con esa feature (Render no la tiene nativa).

---

_Última actualización: Abril 2026 — Fase 10.6 (Auto-asignación de vídeos YouTube) completada. Infra: Render + Vercel + Neon (migrada desde Railway)._
