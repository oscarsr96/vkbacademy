  ...
  return (
    <div className="app-shell">
      ...
      <div className="app-topbar">
        <button className="app-hamburger" onClick={() => setMenuOpen(true)} aria-label="Abrir menú">☰</button>
        <span className="app-topbar-brand">🏀 {brandName}</span>
        <button className="app-topbar-logout" onClick={() => logout()} disabled={isPending} title="Cerrar sesión">↩</button>
      </div>

      {menuOpen && <div className="app-overlay" onClick={() => setMenuOpen(false)} />}

      {/* Sidebar */}
      <aside className={`app-sidebar${menuOpen ? ' open' : ''}`} style={{ ...baseStyles.sidebar, borderRight: `1px solid ${c}26` }}>
        <button className="app-sidebar-close" onClick={() => setMenuOpen(false)} aria-label="Cerrar menú">✕</button>
        ...
        <div style={{ ...baseStyles.brand, borderBottom: `1px solid ${c}1f` }}>
          ...
          <span style={{ color: c, fontWeight: 800, fontSize: '0.9rem', textAlign: 'center' }}>{brandName}</span>
        </div>
        ...
        <nav style={baseStyles.nav}>
          {links.map(({ to, label, end, divider }, i) => (
            <div key={to}>
              {divider && i > 0 && <div style={{ height: 1, background: `${c}1a`, margin: '8px 6px' }} />}
              <NavLink
                to={to}
                end={end}
        ...
        <div style={{ ...baseStyles.userSection, borderTop: `1px solid ${c}1f` }}>
          ...
          <button
            ...
            title="Cerrar sesión"
            onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.color = c; }}
...
const baseStyles: Record<string, React.CSSProperties> = {
  sidebar: {
    ...
    padding: '24px 14px', gap: 6,
    position: 'sticky', top: 0, height: '100vh',
  },
  brand: {
    display: 'flex', alignItems: 'center', gap: 10,
    padding: '0 8px 20px', marginBottom: 8, flexDirection: 'column',
  },
  ...
  userSection: {
    display: 'flex', alignItems: 'center', gap: 10,
    padding: '14px 8px 0', marginTop: 8,
  },
  userInfo: {
  ...
  logoutBtn: {
    ...
    fontSize: '1.125rem', padding: 4, borderRadius: 6,
